        ...
        classNode.lambdaHelper(true) {
            ...
            if (isNotEmpty()) {
                classNode.methods?.forEach { method ->
                    if (method.name == "<init>") {
                        forEach {
                            val hasStatic = it.access and ACC_STATIC != 0
                            initFunction(classNode, method, it.name, hasStatic)
                        }
                        return@forEach
                    }
                }
            ...
            insertTrack(classNode, method, method.access and ACC_STATIC != 0)
        }
//        }
    ...
    private fun initFunction(node: ClassNode, method: MethodNode, methodName: String = "", isStatic: Boolean = false) {
        var hasDoubleTap = false
        val variableName = if (isStatic) "doubleTapStatic_${methodName.replace("-", "_")}" else "doubleTap"
        node.fields?.forEach {
    ...
    private fun insertTrack(node: ClassNode, method: MethodNode, isStatic: Boolean = false) {
        ...
        val variableName = if (isStatic) "doubleTapStatic_${method.name.replace("-", "_")}" else "doubleTap"
        // 判断方法名和方法描述
        val instructions = method.instructions
        ...
        instructions?.insertBefore(
                ...
                DoubleTabConfig.ByteCodeInjectFunctionName, if (timeCheck != null) "(I)Z" else "()Z", false
        )
        )
